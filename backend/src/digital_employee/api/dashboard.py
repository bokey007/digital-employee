...
async def get_metrics(db: AsyncSession = Depends(get_db)):
    ...
    # Emails Received
    # Assuming any audit log actor that is not system or admin corresponds to an incoming email trigger
    received_q = await db.execute(
        select(func.count(AuditLog.id)).where(
            AuditLog.actor.notin_(["system", "admin"])
        )
    )
    emails_received = received_q.scalar() or 0

    # Emails Sent
    # These explicit audit actions indicate a distinct outgoing email generated by the system.
    ...
    # Value Calculation
    # - 10 minutes (0.166h) per email parsed / sent
    # - 15 minutes (0.25h) per chatbot question answered
    email_hours = (emails_sent + emails_received) * (10 / 60)
    chat_hours = questions_answered * (15 / 60)
    
    # Additionally calculate processing time saved per completed task
    content_reworded = await db.execute(select(func.count(AuditLog.id)).where(AuditLog.action == AuditAction.CONTENT_REWORDED))
    reword_hours = (content_reworded.scalar() or 0) * (20 / 60) # 20 mins per section rewrite
    
    consolidated = await db.execute(select(func.count(AuditLog.id)).where(AuditLog.action == AuditAction.NEWSLETTER_CONSOLIDATED))
    consolidation_hours = (consolidated.scalar() or 0) * (30 / 60) # 30 mins per full consolidation
    
    hours_saved = round(email_hours + chat_hours + reword_hours + consolidation_hours, 1)
    
    # Assuming blended internal cost of $65 USD / hour
